...
class BigQueryFieldExtractorNestedArrayTest extends AnyFlatSpec with Matchers {
  ...
  it should "extract generic records in an array" in {
    ...
    fn(testArrayBqRecord) should be (testArrayBqRecord.get("innerArrayRoot"))
  }
  ...
  it should "extract a field from generic records in an array" in {
    ...
    fn(testArrayBqRecord) should be (
      testArrayBqRecord
  ...
  it should "flatten resulting array with a nullable field in the path" in {
    ...
    fn(testArrayBqRecord) should be (expected)
  }
}

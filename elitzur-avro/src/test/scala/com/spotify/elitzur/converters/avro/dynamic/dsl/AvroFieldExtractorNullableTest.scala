...
import com.spotify.elitzur.converters.avro.dynamic.dsl.helpers.SampleAvroRecords.innerNestedSample
...
class AvroFieldExtractorNullableTest extends AnyFlatSpec with Matchers {
  ...
  it should "extract a null from an Union schema type" in {
    ...
    // Output: null
    val fn = new FieldAccessor(TestAvroUnionTypes.SCHEMA$).getFieldAccessor(".optRecord.optString")
  ...
  it should "return null if child schema is non-nullable" in {
    // Input: {"optRecord": null}
    // Output: "null"
    val fn = new FieldAccessor(TestAvroUnionTypes.SCHEMA$).getFieldAccessor(".optRecord.nonOptString")
    val testNullRecord = TestAvroUnionTypes.newBuilder().setOptRecord(null).build
  ...
  it should "return the elements of an array if array is not null" in {
    ...
    fn(testInnerNonNullRecord) should be (
  ...
  it should "return null if array is null" in {
    ...
    fn(testInnerNullRecord) should be (

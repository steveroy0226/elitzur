...
import com.spotify.elitzur.converters.avro.dynamic.dsl.helpers.SampleAvroRecords.testAvroArrayTypes
...
class AvroFieldExtractorNestedArrayTest extends AnyFlatSpec with Matchers {
  ...
  it should "extract generic records in an array" in {
    ...
    val fn = new FieldAccessor(TestAvroArrayTypes.SCHEMA$).getFieldAccessor(".innerArrayRoot[]")
  ...
  it should "extract a field from generic records in an array" in {
    // Input: {"innerArrayRoot": [{"userId": "one"}, {"userId": "two"}]}
    // Output: ["one", "two"]
    val fn = new FieldAccessor(TestAvroArrayTypes.SCHEMA$).getFieldAccessor(".innerArrayRoot[].userId")
  ...
  it should "extract a field from nested generic records in an array" in {
    ...
    //    ]}
    // Output: [-1, -5]
    val fn = new FieldAccessor(TestAvroArrayTypes.SCHEMA$)
